...
export const FrontDeskBillingPOS: React.FC<FrontDeskBillingPOSProps> = ({ queue = [], canWrite = true }) => {
  ...
  const handleSubmitPayment = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!canWrite) {
      alert('Action Blocked: Front-desk payment collection is disabled by Administrator (Write = OFF).');
      return;
    }
  ...
  const handleConfirmRedeemSession = async () => {
    if (!canWrite) {
      alert('Action Blocked: Package redemption is disabled by Administrator (Write = OFF).');
      return;
    }
  ...
  const handleConfirmTopUpWallet = async () => {
    if (!canWrite) {
      alert('Action Blocked: Wallet top-up is disabled by Administrator (Write = OFF).');
      return;
    }
  ...
  const handleConfirmRefund = async () => {
    if (!canWrite) {
      alert('Action Blocked: Refund processing is disabled by Administrator (Write = OFF).');
      return;
    }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-6">
        ...
        <div className="lg:col-span-5 bg-white dark:bg-[#1E2718] border border-slate-200 dark:border-[#2F3E29] rounded-2xl p-5 shadow-sm space-y-4">
          ...
          <form onSubmit={handleSubmitPayment} className="space-y-4">
            ...
            <button
              type="submit"
              disabled={isSubmitting || !selectedPatientId || !canWrite}
              className={`w-full py-3 px-4 rounded-xl font-bold text-sm text-white flex items-center justify-center gap-2 transition-all shadow-md ${
                !canWrite
                  ...
                  : 'bg-emerald-600 hover:bg-emerald-700 disabled:opacity-50 disabled:cursor-not-allowed shadow-emerald-600/20 cursor-pointer active:scale-[0.99]'
              }`}
            >
              {!canWrite ? (
                <>
                  <Lock className="w-4 h-4" />
                  <span>Payment Collection Locked (Read-Only)</span>
                </>
              ) : isSubmitting ? (
