...
export const PharmacyWorkspace: React.FC<PharmacyWorkspaceProps> = ({
  ...
  const handleCreatePO = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!pharmaProcurePerms.canWrite) {
      alert('Action Blocked: PO generation is disabled by Administrator (Write = OFF).');
      return;
    }
        ...
        <div className="space-y-6">
          {!pharmaProcurePerms.canWrite && (
            <div className="p-4 rounded-2xl bg-amber-500/10 border border-amber-500/30 text-amber-800 dark:text-amber-200 text-xs flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
                <Lock className="w-4 h-4 text-amber-600 shrink-0" />
                <span><strong>Read-Only Mode Active:</strong> Creating and receiving purchase orders is locked because Administrator has revoked write permissions for Procurement & Restock.</span>
              </div>
              <span className="text-[10px] font-bold px-2 py-0.5 rounded bg-amber-500/20 text-amber-700 dark:text-amber-300">View Only</span>
            </div>
          )}
          <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
            ...
            {pharmaProcurePerms.canWrite ? (
              ...
              <button
                type="button"
                disabled
                className="px-4 py-2 rounded-xl bg-slate-400 dark:bg-slate-700 text-white font-bold text-xs cursor-not-allowed opacity-75 flex items-center gap-2 self-start sm:self-auto"
              >
                <Lock className="w-4 h-4" />
                <span>Create PO (Locked)</span>
              </button>
            )}
          </div>
