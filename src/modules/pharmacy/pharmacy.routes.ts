...
router.post('/dispense', requireModulePermission('pharma_queue', 'write'), (req, res, next) =>
  pharmacyController.dispensePrescription(req, res, next)
);

// Drug Inventory Vault
router.get('/inventory', (req, res, next) => pharmacyController.getInventory(req, res, next));
router.post('/inventory', requireModulePermission('pharma_inventory', 'write'), (req, res, next) =>
  pharmacyController.saveMedicine(req, res, next)
);

// OTC Point of Sale (POS) Billing
router.post('/pos', requireModulePermission('pharma_pos', 'write'), (req, res, next) =>
  pharmacyController.processPosSale(req, res, next)
);
